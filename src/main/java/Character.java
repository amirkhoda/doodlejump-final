...
public class Character extends Pane{
    ...
    public void moveY(int value){
        ...
        for(int i = 0; i < Math.abs(value); i++){
            for(Platform platform : PaneOrganizer.platforms){
                ...
                for(Hole hole : PaneOrganizer.holes){
                   if (getBoundsInParent().intersects(hole.getBoundsInParent())){
                        if (this.getTranslateY() + this.getHeight() == hole.getTranslateY() + hole.getHeight()) {
                            System.out.println("Game Over");
                            Game.getGameInstance().gameOver(true);
